...
def bgs_library_method (dir):
  ...
  video_list = ["./Clip "+dir +"/" + dir +'.mp4',"./Clip "+dir +"/" + dir + " Deinterleizing.mp4"]
    ...
    for algorithm in algorithms:
      ...
      if(image):
        ...
        for x in range(0, len(img_array)):
          ...
          #out = cv2.VideoWriter(str(algorithm.__class__),cv2.VideoWriter_fourcc(*'mp4v'),29,(280,260))
          count = 10;
          flag, frame = capture.read()
          #height, width,_  = frame.shape
          #video = cv2.VideoWriter("P"+".mp4",cv2.VideoWriter_fourcc(*'mp4v'),15,(width,height))
          cap=[]
          while True:
            flag, frame = capture.read()
            ...
            if flag:
              ...
              #cv2.imshow(str(algorithm.__class__), img_output)
              if(count < 100):
                count = count+1   
              cap.append(img_output) 
              #video.write(img_output)
              #cv2.imwrite(str(algorithm.__class__)+"%d.jpg" % count, img_output)
              #cv2.imshow(str(algorithm.__class__), img_bgmodel)
            # out = cv2.VideoWriter(str(algorithm.__class__),cv2.VideoWriter_fourcc(*'mp4v'),29,size)
          ...
          height, width  = cap[-1].shape
          video = cv2.VideoWriter(video_file + str(algorithm.__class__)+'.mp4',cv2.VideoWriter_fourcc(*'mp4v'),30,(width,height),False)
          l = len(cap)-1
          count =0
          while (l>=count):
              video.write(cap[count])
              count =count+1
